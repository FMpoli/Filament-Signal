@php
    /** @var \Base33\FilamentSignal\Models\SignalAction $action */
...
<div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-white/10 dark:bg-white/5">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div class="flex-1">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                {{ __('voodflow::signal.actions.action_number', ['number' => $executionOrder, 'name' => $name]) }}
            </h3>
            <div class="mt-1 flex items-center gap-2">
                <span class="inline-flex items-center rounded-md {{ $typeColor }} px-2.5 py-0.5 text-xs font-medium uppercase tracking-wide">
                    {{ $actionType }}
                </span>
                @if(!$isActive)
                    <span class="inline-flex items-center rounded-md bg-gray-100 px-2 py-1 text-xs font-medium text-gray-600 dark:bg-gray-700 dark:text-gray-300">
                        {{ __('voodflow::signal.options.status.disabled') }}
                    </span>
                @endif
            </div>
        </div>
    ...
    @if ($actionType === 'webhook')
        <div class="mb-6 space-y-4">
            <h4 class="text-sm font-semibold text-gray-900 dark:text-gray-100">
                {{ __('voodflow::signal.sections.destination') }}
            </h4>
            <div class="grid gap-4 md:grid-cols-3">
                <div class="md:col-span-2">
                    ...
                    <div class="break-all font-mono text-sm text-gray-900 dark:text-gray-100 bg-gray-50 dark:bg-gray-800/50 px-3 py-2 rounded-md">
                        {{ $configuration['url'] ?? '—' }}
                    </div>
                </div>
                <div>
                    <div class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-300 mb-1">
                        {{ __('voodflow::signal.fields.http_method') }}
                    </div>
                    <span class="inline-flex items-center rounded-md bg-green-100 px-3 py-2 text-sm font-semibold text-green-700 dark:bg-green-900/50 dark:text-green-200">
            ...
            @if(isset($configuration['verify_ssl']) && $configuration['verify_ssl'])
                <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <span>{{ __('voodflow::signal.fields.ssl_verified') }}</span>
                </div>
            @endif
        </div>
    @endif
    ...
    @if(isset($configuration['payload_config']))
        ...
        <div class="space-y-4">
            <h4 class="text-sm font-semibold text-gray-900 dark:text-gray-100">
                {{ __('voodflow::signal.fields.payload_configuration') }}
            </h4>
            ...
            @if(!empty($includeFields))
                <div>
                    <div class="mb-2 text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-300">
                        {{ __('voodflow::signal.fields.included_fields') }}
                    </div>
                    <div class="flex flex-wrap gap-1.5">
                        @foreach($includeFields as $field)
                            <span class="inline-flex items-center rounded-md bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-700 dark:bg-blue-900/50 dark:text-blue-200">
                                {{ $field }}
                            </span>
                        @endforeach
                    </div>
                            ...
                            <div>
                                ...
                                <div class="flex flex-wrap gap-1.5">
                                    @foreach($fields as $field)
                                        <span class="inline-flex items-center rounded-md bg-purple-50 px-2.5 py-1 text-xs font-medium text-purple-700 dark:bg-purple-900/50 dark:text-purple-200">
                                            {{ $field }}
                                        </span>
                                    @endforeach
                                </div>
    ...
    @if($actionType === 'log' && empty($configuration['payload_config']))
        <div class="mt-4 rounded-lg bg-purple-50/50 p-4 text-sm text-purple-700 dark:bg-purple-900/40 dark:text-purple-200">
            {{ __('voodflow::signal.helpers.log_configuration') }}
        </div>
    @endif
</div>
